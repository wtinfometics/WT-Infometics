            ...
            <div class="row g-4"> <!-- Featured Blog -->
                <div class="col-lg-6 wow fadeInLeft" data-wow-delay="0.2s">
                    <div class="featured-blog">
                        <div class="blog-image"> <img src="{{ !empty($data[0]->featured_image) ? asset($data[0]->featured_image) : '' }}" alt=""> <span class="category"> 
                                {{$data[0]->categories->category_name}} </span> </div>
                        <div class="blog-content">
                            <div class="blog-meta"> <span> <i class="far fa-user"></i> WT Infometics </span> <span> <i
                                        class="far fa-calendar-alt"></i> {{$data[0]->created_at->format('d F Y') }}</span> </div>
                            <h3><a href="{{ url('blogs/' . $data[0]->post_slug) }}"> {{ \Illuminate\Support\Str::limit($data[0]->post_title, 75, '…') }} </a></h3>
                            <p> {{ \Illuminate\Support\Str::limit($data[0]->short_description, 200, '…') }} </p> 
                            <a href="{{ url('blogs/' . $data[0]->post_slug) }}" class="read-more-btn"> Read Full Article <i class="fas fa-arrow-right"></i> </a>
                        </div>
                    </div>
                </div> <!-- Blog Cards -->
                <div class="col-lg-6">
                    <div class="row g-4"> <!-- Blog 1 -->
                    @foreach($data->skip(1)->take(4) as $blog)
<div class="col-12 wow fadeInRight" data-wow-delay="0.3s">
    <div class="mini-blog-card">
        <img src="{{ !empty($blog->featured_image) ? asset($blog->featured_image) : '' }}" alt="{{ $blog->post_title }}">
